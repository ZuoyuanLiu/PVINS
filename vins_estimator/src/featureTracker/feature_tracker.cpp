...
void reduceVector(vector<int> &v, vector<uchar> status)//实现将1保留，然后0都放到最后面，最后resize一下就是实现了删除所有为0的位置的数
{
    int j = 0;
    for (int i = 0; i < int(v.size()); i++)
        if (status[i])
...
map<int, vector<pair<int, Eigen::Matrix<double, 7, 1>>>> FeatureTracker::trackImage(double _cur_time, const cv::Mat &_img, const cv::Mat &_img1)
{
    ...
    */
    ...
    if (prev_pts.size() > 0) 
    {
        ...
        if(hasPrediction)//有预测结果，使用1层金字塔
        {
            ...
            if (succ_num < 10)//小于10时，输入的基于最大金字塔层次数为3
    ...
    cur_un_pts = undistortedPts(cur_pts, m_camera[0]); //去畸变并归一化相机平面
    ...
    for (size_t i = 0; i < ids.size(); i++)
    {
        ...
        featureFrame[feature_id].emplace_back(camera_id,  xyz_uv_velocity);//emplace_back和push_back一样的，效率高点
